...
function getImageUrlDashboardImproved(property) {
  ...
  const portadaImage = property.imagenes.find(img => img.esPortada) || property.imagenes[0];
...
mockDashboardResponse.data.properties.forEach((property, index) => {
  ...
  console.log(`🔧 Función ACTUAL: ${currentResult || '❌ NULL (Sin imagen disponible)'}`);
  console.log(`✅ Función MEJORADA: ${improvedResult || '❌ NULL (Sin imagen disponible)'}`);
  ...
  if (shouldShowImage && !currentResult) {
    console.log('🚨 PROBLEMA: Debería mostrar imagen pero la función actual devuelve NULL');
...
console.log('\n🚀 PRÓXIMOS PASOS:');
console.log('1. Verificar qué URLs llegan realmente al dashboard tras actualización');
console.log('2. Comparar con las URLs que funcionan en el formulario de edición');
