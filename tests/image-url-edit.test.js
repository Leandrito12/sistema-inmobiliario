...
console.log(JSON.stringify(mockPropertyResponse.data.property.imagenes, null, 2));
...
try {
  ...
  currentUrls.forEach((url, index) => {
    ...
    console.log(`  Imagen ${index + 1}: ${isValid ? '✅ VÁLIDA' : '❌ INVÁLIDA'} - ${url}`);
  });
  ...
  console.log('\n🎯 RESULTADO:');
  const allValid = correctUrls.every(url => url.startsWith('http://localhost:5001'));
  if (allValid) {
